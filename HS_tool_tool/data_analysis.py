...
class CSV2pd:
    ...
    def __init__(self, path):
        self.path = self.path = copy.deepcopy(path)
        self.df = pandas.read_csv(path, header=0)
...
class Picture:
    def __init__(self, item_info, key, data, picture_info):
        ...
        self.subplot_height_inch = 3  # 一个子图高度
...
class Multi_picture(Picture):
    def __init__(self, item_info, key, data, picture_info):
        ...
        self.ymax = max(max_l)
        self.ymin = min(min_l)
        # print('y max is ', self.ymax, self.ymin)
        self.point_size = 2
    ...
    def get_xy(self, key):
        if len(key) == 1:
            key = key[0]
        y = self.data.get_group(key)[self.item_info["name"]]  # 这里的y不是数字类型
    ...
    def set_xtop_title_table(self, ax, i, j, k, title_loc, show_list=None):
        ...
        if (j == 0) or (self.key[i * self.ncol + j][k] != self.key[i * self.ncol + j - 1][k] and j > 0):
            ...
            # 表格竖线，需要计算是否该画线和线的高度
            if self.is_ver_line(k, i, j):
                if j != 0:  # 第一个子图的刻度线不隐藏, 然后自己在坐标轴上画线
                    ax.spines['left'].set_color('none')
                line_width = 0.7 * (len(self.key[0]) - k) if j != 0 else 1
                color = "black" if j == 0 else (140 / 255, 140 / 255, 140 / 255)
                ax.axvline(ax.get_xlim()[0], ymin=0, ymax=top,
                           linewidth=line_width, linestyle='-', color=color, clip_on=False)
        # 表格横线
        ...
        for i in range(self.nrow):
            for j in range(self.ncol):
                if i * (self.ncol) + j >= len(self.key):
                    break
                ax = self.get_ax(i, j)
                # todo get the limit info from item info
                ax.axhline(8, linewidth=1, linestyle='-', color="red")
                ax.axhline(36, linewidth=1, linestyle='-', color="red")
                [x, y] = self.get_xy(self.key[i * self.ncol + j])
                ax.plot(x, y, marker="o", markersize=self.point_size, linestyle="", color="blue")
...
def main(path, item, option_key, picture_type="point"):
    # 这里由GUI生成
    # path = "C:\\Users\\蓝海梦\\Desktop\\py_envir\\HS_4.1\\test_data\\insight_data.csv"
    # path = "/Users/js-15400155/Desktop/GRR.csv"
    # path = '/storage/0123-4567/qpython/insight.csv'
    # option_dict = [{"Version": []}, {"Station ID": []}, {"SerialNumber": []}]
    # option_key = [j for i in option_dict for j in i]
    result = None
    csv2pd = CSV2pd(path)
    if not option_key or option_key == [""]:
        # 手动添加一列option
        csv2pd.df.insert(loc=0, column='--all--', value="all")
        option_key = ['--all--']
    group_data = csv2pd.group_data(option_key)
    group_key = csv2pd.group_key(group_data)
    print(group_key)
    item_info = {"name": item, "upper_limit": '', "lower_limit": '', "unit": "NA"}
    picture_info = {}
...
path = "C:\\Users\\蓝海梦\\Desktop\\py_envir\\HS_4.1\\test_data\\insight.csv"
item = "AC_CP"
main(path, item, ["SerialNumber", "Station ID", "Version", "Site", "Special Build Name"])