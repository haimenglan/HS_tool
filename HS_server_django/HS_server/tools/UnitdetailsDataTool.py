...
class UnitdetailsData:
    ...
    def read_csv(self,file_path):
        csv_line = []
        with open(file_path) as csvf:  # encoding='utf-8'
    ...
    def show_empty_fail(self, original_csv_line):
        ...
        csv_line_multi = self.read_csv(csv_file_path_multi)
...
class Html_xls:
    ...
    def add_table_to_sheet(self, sheet_name, table_name, table, table_name_style='margin-top:30px;',
        ...
        if sheet_name not in self.xls:
            ...
            self.xls[sheet_name] = ""
            # self.xls[sheet_name] = f"<div style='{table_name_style}'>{table_name}</div>"
            # self.xls[sheet_name] += f"<div style='{table_style}'>{table}</div>"
        # else:
        self.xls[sheet_name] += f"<div style='{table_name_style}'>{table_name}</div>"
        self.xls[sheet_name] += f"<div style='{table_style}'>{table}</div>"
...
class Unitdetails_xls(Haimeng_xlwt):
    def __init__(self, xls_path, unitdetailsData):
        ...
        self.html_table = "<table border=1 cellspacing=0>content</table>"

        self.daily_report_html = [
            {"performance_breakdown": Html_table()},
            {"yield": Html_table()},
            {"retest": Html_table()},
            {"summary": Html_table()}
        ]
        ...
        for i, value in enumerate(self.daily_report_html):
            for each_table in value:
                self.html_xls.add_table_to_sheet("daily_report", each_table, value[each_table].table)
        self.html_xls.finish_add_sheet("daily_report")
        for i,value in enumerate(self.other_html):
            for each_table in value:
                self.html_xls.add_table_to_sheet(each_table, each_table, value[each_table].table)
                self.html_xls.finish_add_sheet(each_table)
    ...
    def write_Performance_Breakdown(self):
        ...
        self.daily_report_html[0]["performance_breakdown"].write_header(title)
        row = 2
        ...
        for each_station in self.unitdetailsData.STATION_LIST:
            ...
            self.daily_report_html[0]["performance_breakdown"].write_line([each_station]+line)
            row+=1
    ...
    def write_Yield(self, is_platinum=False):
        ...
        self.write_line(sheet,1,0,title,self.gray_title_style)
        self.daily_report_html[1]["yield"].write_header(title)
        row = 2
        ...
        for each_station in self.unitdetailsData.STATION_LIST:
            ...
            for each_line in all_line:
                ...
                self.daily_report_html[1]["yield"].write_line(each_line)
                row+=1
    ...
    def write_retest(self, is_platinum=False):
        ...
        self.write_line(sheet,1,0,title,self.gray_title_style)
        self.daily_report_html[2]["retest"].write_header(title)
        row = 2
        ...
        for each_station in self.unitdetailsData.STATION_LIST:
            ...
            for each_line in all_line:       
                self.write_line(sheet,row,0,each_line,self.normal_style)
                self.daily_report_html[2]["retest"].write_line(each_line)
                row+=1
    ...
    def write_summary(self):
        ...
        self.write_line(sheet,1,0,title,self.gray_title_style)
        self.daily_report_html[3]["summary"].write_header(title)
        row = 2
        ...
        for each_station in self.unitdetailsData.STATION_LIST:
            ...
            self.daily_report_html[3]["summary"].write_line(line)
            row+=1
        #self.set_blank(sheet,1,row,0,len(title))
    ...
    def write_station_id_config(self,key_type):
        ...
        for each_column in width:
            sheet.col(each_column).width = width[each_column]
        self.other_html.append({key_type: Html_table()})
        if key_type=="station":
        ...
        for each_station in self.unitdetailsData.STATION_LIST:
            for each_item in self.unitdetailsData.ITEM_LIST[each_station]:
                ...
                for each_line in all_line:
                    self.write_line(sheet,row,0,each_line,self.normal_style)
                    self.other_html[-1][key_type].write_line(each_line)
                    row+=1
        #self.set_blank(sheet,1,row,0,len(title))
    ...
    def write_line_breakdown(self):
        ...
        self.other_html.append({"Line Break Down":Html_table()})
        self.write_line(sheet,1,0,title,self.gray_title_style)    
        row = 2
        ...
        for each_station in self.unitdetailsData.STATION_LIST:
            ...
            for each_line in all_line:
                self.write_line(sheet,row,0,each_line,self.normal_style)
                self.other_html[-1]["Line Break Down"].write_line(each_line)
                row+=1
        #self.set_blank(sheet,1,row,0,len(title))
...
def main(csv_path, xls_save_path, show_empty_mode = False, is_change_config = False, is_FBR_config = False,
         FBR_xls_file="",csv_file_path_multi="", is_platinum=False):
    if csv_path != "":
...
def run_daily_report(csv_path, xls_save_path, show_empty_mode = False,is_change_config = False,is_FBR_config = False,
    ...
    run_daily_report_process = Process(target=main,args=(csv_path,xls_save_path,show_empty_mode,is_change_config,is_FBR_config,
                                                         FBR_xls_file,csv_file_path_multi,is_platinum, ))
